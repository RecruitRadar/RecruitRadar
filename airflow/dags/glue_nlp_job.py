...
with DAG('glue_nlp_job_dag',
         description='Run Glue NLP Job',
         schedule_interval=None,
         start_date=datetime(2023, 8, 15),
         default_args=DEFAULT_ARGS) as dag:

    @task()
    def start_nlp_job(job_name: str) -> str:
    ...
    @task()
    def monitor_nlp_job(job_name: str, job_run_id: str) -> str:
        glue_client = create_glue_client()
        response = glue_client.get_job_run(JobName=job_name, RunId=job_run_id)
        job_run_state = response['JobRun']['JobRunState']

        while job_run_state in ['RUNNING', 'STOPPING']:
            print(f"NLP Job status: {job_run_state}")
            time.sleep(10)
            response = glue_client.get_job_run(JobName=job_name, RunId=job_run_id)
            job_run_state = response['JobRun']['JobRunState']

            if job_run_state == 'FAILED':
                raise Exception(f"NLP Job failed with status: {job_run_state}")
    ...
    def log_final_message(job_run_state:str) -> None:
        print(f"NLP Job finished with status with {job_run_state}!")
        
        return f"NLP Job finished with status with {job_run_state}!"
        
    nlp_job_name = 'de1_1_2nd_preprocessing_script'
    job_run_id_extract = start_nlp_job(job_name=nlp_job_name)
    monitor_task = monitor_nlp_job(job_name=nlp_job_name, job_run_id=job_run_id_extract)
